...
?>
    <div class="card margins">
            ...
            <div class="card-header">
                <h3 class="mb-0 float-right">كتب صوتية</h3>
                <div>
                  <button type="button" id="services_modal" class="float-left btn btn-danger mb-3 add_services" data-toggle="modal" data-target="#modal-service"><i class="fa fa-plus">اضافة كتاب صوتي </i></button>
                </div>
            </div>
            <?php show_messages() ?>
            <div class="table-responsive py-4">
              <div id="datatable-basic_wrapper" class="dataTables_wrapper dt-bootstrap4">
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" id="<?php echo $value->id ?>" class="del_click btn  btn-icon btn-secondary remove_services" data-toggle="modal" data-target="#modal_delete">
...
<script>
    ...
    $(document).on('click', '.update_services', function () {
        ...
        $.ajax({
            ...
            method: "POST",
            data: { id: id , type : 'audio_books' },
            dataType: "json",
            success: function (data) {
                $('#table_for_add').val("audio_books");
                $('#book_name').val(data.book_name);
		        $('#book_link').val(data.audio);
		        $('#description').val(data.description);
                $('#language').val(data.language);
		        $('#author').val(data.author);
		        $('#image').val(ReadersCommunity/images/data.img);
