...
$u_id=isset($_GET['adv_id']) && is_numeric($_GET['adv_id']) ? intval($_GET['adv_id']):0;

$qur="select Full_Name,img from users where `id` ='$u_id'";
$r=mysqli_query($con,$qur);
$info=mysqli_fetch_assoc($r);


$nameUser="".$info['Full_Name']."";
$pic="images/".$info['img']."";
...
<div id='frame'>
	...
	<div id='sidepanel'>
	 <div><a href='http://localhost/ReadersCommunity/Advertisments.php'><p style='color:lightgreen;font-size:30px;margin-left:20px'class='glyphicon glyphicon-arrow-left'></p></a>
      </div>
		<div id='profile'>
			<div class='wrap'>
				<img id='profile-img' src='images/".$_SESSION['userimage']."' class='online' alt='' />
		...
		if(!(mysqli_num_rows($res)>0)){ //ما في محادثة سابقة
			...
			echo"<ul id='ul-users'>	<li class='contact active ' id='cont_users' num='".$u_id."' name='".$name['Full_Name']."' data-id='images/".$name['img']."'>
					<div class='wrap'>
			...
			if( isset($arr[$i]) !=''  ){
              $number =$arr[$i];
			   ...
			   if(mysqli_num_rows($na2)>0){
				   ...
				   if($arr[$i]==$u_id){
					   echo"<li class='contact active' id='cont_users' num='".$arr[$i]."' name='".$name2['Full_Name']."' data-id='images/".$name2['img']."'>
					<div class='wrap'>
...
function newMessage(glob) {
	...
	if($.trim(message) == '') {
	 ...
	 $.ajax({
			  method:"POST",
			  url:"send_receive.php",
			  data:{message:message,glob:glob},
			  success:function(){
...
$(document).on('click','#cont_users',function(){
	var num=$(this).attr('num');
	globalID = num;
	
	$('#ul-users li').removeClass('active');
...
window.value=<?php echo $u_id; ?>;


$( window ).on( "load",recv(window.value));
...
function recv(num){
	 //if(num==undefined){num=<?php echo $u_id; ?>;}
